...
const PORTALS: Portal[] = [
  ...
  },
  {
    key: "staff",
    title: "Staff Portal",
    description: "Access attendance sheet and student database quickly and easily.",
    features: ["Attendance Sheet", "Student Database", "Placement Updates", "Communication"],
    role: "TEACHER",
    accent: "hover:border-amber-600 text-amber-700",
  ...
  if (selected && !signedInRole) {
    return (
      <div className="flex flex-col items-center bg-white p-12 rounded-2xl shadow-sm border border-stone-200 max-w-md w-full mx-auto">
        ...
        <button
          onClick={() => signIn("google", { callbackUrl: `/router?role=${selected.role}` })}
